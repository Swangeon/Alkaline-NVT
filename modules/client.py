...
class ClientSessionHandler(object):
	...
	def __output_data(self, response, cwd) -> str:
		"""
		...
		if type(response) == str:
			...
			# When you change directories, the server sends back the 'Changed Direcotry' information and then the actual path
	...
	def __download(self, cmd_split: list) -> None:
		"""
		...
		try:
			...
			while True:
				...
				if cmd_split[0].lower() == "upload":
					try:
						file = self.__upload(cmd_split[1])
